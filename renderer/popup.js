...
const I18N = {
  en: {
    tab_img2pdf:'Img→PDF', tab_pdf2img:'PDF→Img', tab_merge:'Merge', tab_math:'Math OCR', tab_ocr:'Text OCR',
    ...
    ocr_checking:'Checking server...',
    ocr_online:'✓ Server online — ready',
    ocr_offline:'✗ Server offline',
    ocr_server_notice:'⚠️ Server not running. First-time setup:',
    ocr_step1:'Install Tesseract OCR engine:',
    ocr_step2:'Install Python packages & start server:',
    ocr_drop_title:'Drop image here',
    ocr_drop_sub:'PNG, JPG — screenshot or photo with text',
    ...
    ocr_lang_jpn:'日本語',
    ocr_lang_kor:'한국어',
    ocr_lang_fra:'Français',
    ocr_lang_deu:'Deutsch',
    ocr_recognize_btn:'Extract Text',
    ocr_result_label:'Extracted Text',
    ocr_copied:'Copied!',
    ocr_err_no_img:'Please select an image first',
    ocr_err_server:'Server error — is the server running?',
    ocr_err_recognize:'Recognition failed',
    ocr_pasted:'Image pasted!',
  },
  zh: {
    tab_img2pdf:'图片→PDF', tab_pdf2img:'PDF→图片', tab_merge:'合并PDF', tab_math:'公式识别', tab_ocr:'文字识别',
    ...
    ocr_online:'✓ 服务器在线 — 就绪',
    ocr_offline:'✗ 服务器离线',
    ocr_server_notice:'⚠️ 服务器未运行，首次使用请安装：',
    ocr_step1:'安装 Tesseract OCR 引擎：',
    ocr_step2:'安装 Python 包并启动服务器：',
    ocr_drop_title:'拖拽图片到此处',
    ocr_drop_sub:'PNG、JPG — 含文字的截图或照片',
    ocr_select_btn:'选择图片',
    ocr_lang_label:'识别语言',
    ocr_lang_eng:'English',
    ocr_lang_chs:'中文（简体）',
    ...
    ocr_lang_fra:'Français',
    ocr_lang_deu:'Deutsch',
    ocr_recognize_btn:'提取文字',
    ocr_result_label:'识别结果',
    ocr_copied:'已复制！',
    ocr_err_no_img:'请先选择图片',
    ocr_err_server:'服务器错误 — 服务器是否已启动？',
    ocr_err_recognize:'识别失败',
    ocr_pasted:'图片已粘贴！',
  }
};
...
function initTextOCR() {
  checkOcrServer();
  $('ocrServerRetryBtn').addEventListener('click', checkOcrServer);

  // Drop zone
  const zone = $('dropZone-ocr');
  zone.addEventListener('dragover', e => { e.preventDefault(); zone.classList.add('drag-over'); });
  zone.addEventListener('dragleave', () => zone.classList.remove('drag-over'));
  zone.addEventListener('drop', e => {
    e.preventDefault(); zone.classList.remove('drag-over');
  ...
  $('selectBtn-ocr').addEventListener('click', e => { e.stopPropagation(); $('fileInput-ocr').click(); });
  $('fileInput-ocr').addEventListener('change', e => {
    const file = e.target.files[0];
    if (file) loadOcrImage(file);
    e.target.value = '';
  });

  $('ocrImgRemove').addEventListener('click', () => {
    ocrImageDataUrl = null;
    $('ocrPreviewImg').src = '';
    $('ocrPreviewWrap').style.display = 'none';
    $('ocrRecognizeBtn').disabled = true;
    $('ocrResult').style.display = 'none';
    $('ocrLangRow').style.display = 'none';
    $('dropZone-ocr').style.display = '';
    $('fileInput-ocr').value = '';
  });

  $('ocrRecognizeBtn').addEventListener('click', recognizeText);

  // Paste support (only when OCR tab active)
  document.addEventListener('paste', e => {
    if (state.currentTab !== 'ocr') return;
    const items = e.clipboardData?.items;
    if (!items) return;
    for (const item of items) {
      if (item.type.startsWith('image/')) {
        e.preventDefault();
        const file = item.getAsFile();
        if (file) { loadOcrImage(file); showToast(t('ocr_pasted'), 'success'); }
        break;
      }
    }
  });
  ...
  $('copyOcrBtn').addEventListener('click', () => {
    ...
    navigator.clipboard.writeText(text).then(() => {
      const btn = $('copyOcrBtn');
      const orig = btn.textContent;
      btn.textContent = t('ocr_copied');
      btn.classList.add('copied');
      setTimeout(() => { btn.textContent = orig; btn.classList.remove('copied'); }, 2000);
    });
  });
...
async function checkOcrServer() {
  const dot    = $('ocrServerDot');
  const label  = $('ocrServerLabel');
  const notice = $('ocrServerNotice');

  dot.className     = 'server-dot checking';
  label.textContent = t('ocr_checking');

  try {
    const resp = await fetch(`${OCR_API}/health`, { signal: AbortSignal.timeout(3000) });
    const data = await resp.json();
    if (data.status === 'ok' && data.ocr_available) {
      dot.className     = 'server-dot online';
      label.textContent = t('ocr_online');
      notice.style.display = 'none';
    } else {
    ...
    dot.className     = 'server-dot offline';
    label.textContent = t('ocr_offline');
    notice.style.display = 'flex';
  }
}
...
function loadOcrImage(file) {
  ...
  reader.onload = e => {
    ...
    $('ocrPreviewImg').src = ocrImageDataUrl;
    $('ocrPreviewWrap').style.display = 'flex';
    $('ocrRecognizeBtn').disabled = false;
    $('ocrResult').style.display = 'none';
    $('ocrLangRow').style.display = 'flex';
    $('dropZone-ocr').style.display = 'none';
  };
  reader.readAsDataURL(file);
}

async function recognizeText() {
  if (!ocrImageDataUrl) { showToast(t('ocr_err_no_img'), 'error'); return; }

  const btn  = $('ocrRecognizeBtn');
  const lang = $('ocrLang').value;
  btn.classList.add('loading');
  btn.disabled = true;

  try {
    const resp = await fetch(`${OCR_API}/ocr`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ image: ocrImageDataUrl, lang }),
      signal: AbortSignal.timeout(30000),
    });

    if (!resp.ok) {
      const err = await resp.json().catch(() => ({}));
      throw new Error(err.error || t('ocr_err_server'));
    }
    ...
    if (err.name === 'TimeoutError') {
      showToast(t('ocr_err_server'), 'error');
    } else {
      showToast(t('ocr_err_recognize') + ': ' + err.message, 'error');
    }
    console.error(err);
  } finally {
    btn.classList.remove('loading');
